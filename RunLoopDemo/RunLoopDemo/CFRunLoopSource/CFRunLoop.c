...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    //通过pthreadPointer作为key直接从dict里获取 说明pthread 和 runloop是一一对应的关系
    CFRunLoopRef loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t));
    __CFUnlock(&loopsLock);
    
   
    if (!loop) { //如果dict里没有，创建一个
...
#pragma mark - ###### CFRunLoopGetMain
CFRunLoopRef CFRunLoopGetMain(void) {
    CHECK_FOR_FORK();
    static CFRunLoopRef __main = NULL; // no retain needed
    //pthread_main_thread_np() 该方法是获取主线程 main pthread
...
#pragma mark - ##### __CFRunLoopRun
/* rl, rlm are locked on entrance and exit */
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {

    int32_t retVal = 0;
      // 开启一个 do while 来保活线程，持续处理事件
    do {
        // 通知Observers -> kCFRunLoopBeforeTimers 接下来将要处理Timer回调
        if (rlm->_observerMask & kCFRunLoopBeforeTimers) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
        // 通知Observers -> kCFRunLoopBeforeSources 接下来将要处理Source0回调
        ...
        // 如果有Source0就处理Source0 Blocks回调
        if (sourceHandledThisLoop) {
            __CFRunLoopDoBlocks(rl, rlm);
        }

        // 如果有Sorce1就跳到handle_msg去处理
        if (__CFRunLoopServiceMachPort(dispatchPort, &msg, sizeof(msg_buffer), &livePort, 0, &voucherState, NULL)) {
            goto handle_msg;
        }
      
        // 如果没有Source1 通知Observers kCFRunLoopBeforeWaiting RunLoop的线程即将进入休眠状态
        ...
        // 通知Observers kCFRunLoopAfterWaiting
...
handle_msg:;
        ...
        // 根据情况 设置对应的retval retVal也决定是runloop是退出还是继续下一个roop
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    ...
    //通知Observers kCFRunLoopEntry
	if (currentMode->_observerMask & kCFRunLoopEntry ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry);
    
    //__CFRunLoopRun(:::::)是RunLoop的核心代码！！！！！
	result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);
    
    //通知Observers kCFRunLoopExit
